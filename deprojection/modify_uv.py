...
if __name__ == '__main__':
    ...
    for _NUM_,_INPUT_ in enumerate(_TEMPARRAY1_):
        ...
        if not _TEMP4_:
            outfile=_TEMPARRAY1_[_NUM_].strip('.txt').strip('.FIT').strip('.FITS').strip('.fit').strip('.fits').strip('.img').strip('.IMAGE').strip('.image').strip('.')
        else:
            outfile=_TEMPARRAY4_[_NUM_].strip('.txt').strip('.FIT').strip('.FITS').strip('.fit').strip('.fits').strip('.img').strip('.IMAGE').strip('.image').strip('.')+'.deproj.fits'
